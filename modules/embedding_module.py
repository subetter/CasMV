...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, neigh_edge, edge_to_time, edge_feat, node_feat, edge_mask=None, sample_ratio=None):
    ...
    src_time_embeddings = self.time_encoder(torch.zeros_like(edge_to_time.to(torch.float32)))
    edge_time_embeddings = self.time_encoder(edge_to_time.to(torch.float32))
