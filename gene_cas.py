...
# twitter  1d
# flags.DEFINE_integer('observation_time', 3600*24*1, 'Observation time.')
# flags.DEFINE_integer('prediction_time', 3600*24*32, 'Prediction time.')

# # path
# flags.DEFINE_string('input', 'dataset/twitter/1d/', 'Dataset path.')
...
# # path
# flags.DEFINE_string('input', 'dataset/weibo/less25/', 'Dataset path.')
...
#
# weibo 1h
flags.DEFINE_integer('observation_time', 1826, 'Observation time.')
flags.DEFINE_integer('prediction_time', 7305, 'Prediction time.')
...
def generate_cascades(ob_time, pred_time, filename, file_train, file_val, file_test, seed):
    ...
    # Important node: for twitter dataset, if you want to compare CasFlow with baselines such as DeepHawkes and CasCN,
    ...
    with open(file_train, 'w') as data_train, \
        ...
        dict1 = {"train": train_cas, "val": val_cas, "test": test_cas}
        with open(FLAGS.input + "dict.pkl", 'wb') as f:
            pickle.dump(dict1, f)
...
def generate_global_graph(file_name, graph_save_path):
    ...
    users = np.array(g.nodes)
    max_user_id = np.max(users.astype(np.int32))
    return max_user_id

import datetime
def generate_diffusion_graph(file_name, graph_save_path, max_user_id):
    g = nx.DiGraph()
    with open(file_name, "r") as f:
        for line in f:
            parts = line.strip().split('\t')
            cas = str(int(parts[0]) + max_user_id)
            pub_time = int(parts[2])
            paths = parts[4].strip().split(' ')
            for path in paths:
                nodes = path.split(':')[0].split('/')
                ...
                if len(nodes) < 2:
                    g.add_node(nodes[-1])
                    # g.add_edge(nodes[-1], cas)
                    g.add_weighted_edges_from([(nodes[-1], cas, int(time) / pub_time)])
                else:
                    g.add_weighted_edges_from([(nodes[-2], nodes[-1], int(time) / pub_time)])
                    g.add_weighted_edges_from([(cas, nodes[-1], int(time) / pub_time)])
                    g.add_weighted_edges_from([(nodes[-2], cas, int(time) / pub_time)])
                    # g.add_edge(nodes[-1], nodes[-2])
                    # g.add_edge(nodes[-2], cas)
                    # g.add_edge(cas, nodes[-1])
        print("Number of nodes in diffusion graph:", g.number_of_nodes())
        print("Number of edges in diffusion graph:", g.number_of_edges())
        with open(graph_save_path, 'wb') as f:
            pickle.dump(g, f)
...
def main(argv):
    ...
    if 'xovee' in FLAGS.input:
        print('Note: this is just a xovee dataset.\n')
    tail_string = '_less25.txt'
    generate_cascades(FLAGS.observation_time, FLAGS.prediction_time,
                      FLAGS.input + 'less25.txt',
                      FLAGS.input + 'train' + tail_string,
                      FLAGS.input + 'val' + tail_string,
                      FLAGS.input + 'test' + tail_string,
                      seed=0)

    max_user_id = generate_global_graph(FLAGS.input + 'less25.txt',
                                        FLAGS.input + 'global_graph_less25.pkl')
    print("max_user_id:",max_user_id)
    # generate_diffusion_graph(FLAGS.input + 'less25.txt',
    #                          FLAGS.input + 'diffusion_graph_less25.pkl', max_user_id)
